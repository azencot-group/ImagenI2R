...
from utils.utils_stl import stl_decompose_batch, reconstruct_from_components
from utils.stl_learned_utils import LearnedSTLComponentStorage
...
def main(args):
    ...
    with CompositeLogger([NeptuneLogger()]) if args.neptune else PrintLogger() as logger:
        ...
        # Initialize Learned STL component storage
        stl_storage = LearnedSTLComponentStorage(device=args.device)
        logging.info("Initialized Learned STL component storage for trend and seasonality.")
        ...
        for epoch in range(init_epoch, args.epochs):
            ...
            for i, data in enumerate(train_loader, 1):
                ...
                # Store trend and seasonal components for later distribution learning
                # Only store during first epoch to capture the full training set once
                if epoch == init_epoch:
            ...
            # Finalize storage after first epoch: Learn parametric distributions
            if epoch == init_epoch and not stl_storage.is_finalized:
                stl_storage.finalize()
                logging.info(f"Learned STL distributions finalized.")
                ...
                with torch.no_grad():
                    with model.ema_scope():
                        ...
                        for data in tqdm(test_loader):
                            ...
                            # === SAMPLE TREND AND SEASONALITY FROM LEARNED DISTRIBUTIONS ===
                            if stl_storage.is_finalized:
                                sampled_trends, sampled_seasonals = stl_storage.sample_learned(batch_size)
                            else:
                                # Fallback to random if not finalized (should not happen in normal flow)
