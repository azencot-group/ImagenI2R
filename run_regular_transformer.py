...
from models.transformer_diffusion import TSTransformerDiffusion
...
def main(args):
    ...
    with CompositeLogger([NeptuneLogger()]) if args.neptune else PrintLogger() as logger:
        ...
        # Initialize the Hybrid Transformer-CNN Diffusion model
        model = TSTransformerDiffusion(args=args, device=args.device).to(args.device)
        ...
        for epoch in range(init_epoch, args.epochs):
            ...
            for i, data in enumerate(train_loader, 1):
                ...
                # Remove index column from irregular data
                x_ts = x[:, :, :-1]

                # We use the clean x_ts from the train_loader for training in regular mode
                # Actually, in run_regular.py, x_ts is the irregular data (noisy)
                # But here we are doing regular diffusion, so we want to learn to denoise
            ...
            if epoch % args.logging_iter == 0:
                ...
                with torch.no_grad():
                    with model.ema_scope():
                        ...
                        # The shape here should be (seq_len, input_channels)
